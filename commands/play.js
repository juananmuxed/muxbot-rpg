...
exports.run = async (client, message, args) => {
    ...
    if (!permissions.has('CONNECT') || !permissions.has('SPEAK')) {
        return musicchannel.send('¡Needed permissions for connect and talk!');
    }
    var opts = {
        maxResults: 1,
    ...
    if (!serverQueue) {
        ...
        try {
            ...
            musicchannel.send(`🎷 Playing: **${song.title}**`)
        ...
        message.delete()
        return musicchannel.send(`**${song.title}** added to queue!, by: **${message.author.tag}**`)
    }
}
